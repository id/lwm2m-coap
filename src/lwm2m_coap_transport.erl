...
go_con_await_aack(Message, State=#state{sendfun=SendFun, cid=ChId})->
...
await_aack({timeout, await_aack}, State=#state{sendfun=SendFun, cid=ChId, msg=BinAck}) ->
    logger:warning("~p <- empty ack ~p [application didn't respond]~n", [self(),BinAck]),
    SendFun(ChId, BinAck),
...
request_complete(Channel, Msg = #coap_message{options=Options}, Ref) ->
    case proplists:get_value(observe, Options, []) of
        [] ->
            % Terminate the transport only if it is not in an observation
            case Ref of
                #{<<"msgType">> := <<"observe">>} -> ok;
                _ -> Channel ! {request_complete, Msg}
            end;
        _Else ->
            ok
    end.
